    ...
    <view class="tabview">
      <scroll-view class="scrollView" scroll-x show-scrollbar="false" :scroll-left="scrollLeft" scroll-with-animation
        ...
        <view class="tabBox" :style="{ 'justify-content': isOutWindow ? '' : 'space-around'}">
          <view class="items" v-for="(item, index) in tabValue" :key="index" @click="$emit('input', index)">
  ...
  export default {
    ...
    methods: {
      ...
      clickTab(index) {
        // 更改选中下标
        this.tIndex = index || 0
        this.$emit("change", this.tIndex)
        // 获取盒子移动距离
        if (this.isOutWindow && this.tIndex >= 0) {
        ...
        uni.createSelectorQuery().in(this).selectAll(".items").boundingClientRect().exec((data) => {
          let width = data[0][this.tIndex].width
          let left = data[0][this.tIndex].left
          let newLeft = 0
          // tab超出屏幕
          if (this.isOutWindow) {
            if (this.tIndex == 0) {
